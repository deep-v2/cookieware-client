...
void FileUtils::validateDirectories()
{
    ...
    //createDirector(getOrphanDir() + "Scripts\\");
    ...
    //createFile(getPraxDir() + "Scripts\\Libs\\");
...
void FileUtils::createFile(const std::string& path)
{
    ...
    if (!directory.empty() && !std::filesystem::exists(directory)) {
        ...
        spdlog::info("Created directory for file: {}", directory);
    }
...
{
    spdlog::info("Starting download from: {} to: {}", url, savePath);
    ...
    HINTERNET hInternet = InternetOpenA("PraxDownloader", INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
    if (!hInternet) {
        spdlog::error("Failed to initialize internet connection");
        return false;
    }
    ...
    // Try to get file size for progress tracking (optional, don't fail if this doesn't work)
    DWORD contentLength = 0;
    DWORD bufferSize = sizeof(contentLength);
    if (!HttpQueryInfoA(hFile, HTTP_QUERY_CONTENT_LENGTH | HTTP_QUERY_FLAG_NUMBER, &contentLength, &bufferSize, NULL)) {
        spdlog::debug("Could not get content length, proceeding without it");
    }

    std::ofstream outFile(savePath, std::ios::binary);
    if (!outFile.is_open()) {
        spdlog::error("Failed to create output file: {}", savePath);
    ...
    if (totalBytesRead > 0) {
        spdlog::info("Download completed successfully. Downloaded {} bytes to: {}", totalBytesRead, savePath);
        return true;
    } else {
        spdlog::error("Download failed - no data received");
        return false;
    }
}
